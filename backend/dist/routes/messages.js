...
// ---------------------------------------------------------------------------
exports.messagesRouter.get("/:id/messages", requireParticipant_js_1.requireParticipant, async (req, res) => {
    ...
    if (req.query["limit"] !== undefined) {
        ...
        if (!Number.isInteger(n) || n < 1 || n > 100) {
            throw (0, errors_js_1.invalidBody)('"limit" must be an integer between 1 and 100.');
        }
        limit = n;
    }
    // Parse cursor
    ...
    let pivot;
    if (beforeId) {
        const found = await prisma_js_1.prisma.message.findUnique({
            where: { id: beforeId },
            select: { id: true, sentAt: true },
        });
        if (!found) {
            throw (0, errors_js_1.invalidBody)('"before" references a message that does not exist.');
        }
        pivot = found;
    }
    ...
    const raw = await prisma_js_1.prisma.message.findMany({
        where,
